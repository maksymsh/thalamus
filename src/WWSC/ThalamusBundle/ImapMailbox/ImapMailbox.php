...
namespace WWSC\ThalamusBundle\ImapMailbox;
/**
...
class ImapMailbox {
    ...
    public function getMail($mailId) {
        ...
        $originalCodeMsg = imap_fetchheader ($this->getImapStream(), $mailId);
        $mailUid = false;
        if(preg_match('/X-Original-To: (\S+)/', $originalCodeMsg, $aMatches)){
    ...
    protected function initMailPart(IncomingMail $mail, $partStructure, $partNum) {
        ...
        elseif(0 == $partStructure->type && $data) {
            if('plain' == strtolower($partStructure->subtype)) {
                ...
                $mail->textPlain .= html_entity_decode(strip_tags($data), ENT_QUOTES, 'UTF-8'); // html to plain text
                $mail->textHtml .= $data;
            }
        }
    ...
    protected function convertStringEncoding($string, $fromEncoding, $toEncoding)
    {
        ...
        return (false !== $convertedString) ? $convertedString : $string;
    }
